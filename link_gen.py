...
make_dir('links')
make_dir('pretrain_labels')


def adjacency_matrix_dgl(g):
    # Ensure the graph is on CPU for numpy compatibility
    g = g.to('cpu')
    
    # Get the edges of the graph
    src, dst = g.edges()
    
    # Convert edge indices to numpy arrays
    src_np = src.numpy()
    dst_np = dst.numpy()
    
    # Create a sparse adjacency matrix
    # Assuming the graph is undirected, edges are added for both directions
    adj_matrix = sp.coo_matrix((np.ones(len(src_np)), (src_np, dst_np)),
...
for k, v in dataset_classes.items():
    ...
    print(type(g))
    adj_train = adjacency_matrix_dgl(g) #adj_train = g.adjacency_matrix()
